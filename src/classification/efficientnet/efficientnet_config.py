"""
efficientnet_config.py
------------------------
Singleton configuration wrapper for EfficientNet classification training.

Identical schema/logic to resnet_config.py — same .ini structure, same
required keys — kept as a separate class (not a reused ResNetConfig
instance) so each architecture's config is its own singleton and doesn't
collide if both pipelines are ever imported in the same process.
...
-----
    ...
    efficientnet_ini = load_config(base_cfg.get("CLASSIFICATION-EFFICIENTNET", "efficientnet.config"))
    cfg              = EfficientNetConfig(efficientnet_ini)
